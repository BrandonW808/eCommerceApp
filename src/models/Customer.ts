...
customerSchema.methods.generateAuthToken = function (this: ICustomer): string {
  ...
  const options: SignOptions = { expiresIn: config.security.jwt.expiresIn as any };

  return jwt.sign(payload, config.security.jwt.secret, options);
};
...
customerSchema.methods.generateRefreshToken = function (this: ICustomer): string {
  ...
  const options: SignOptions = { expiresIn: config.security.jwt.expiresIn as any };
  const token = jwt.sign(payload, config.security.jwt.refreshSecret, options);
...
customerSchema.methods.incrementLoginAttempts = async function (this: ICustomer): Promise<void> {
  ...
  const updates: Partial<ICustomer> = { $inc: { loginAttempts: 1 } } as unknown as Partial<ICustomer>;
